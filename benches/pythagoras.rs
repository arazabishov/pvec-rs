...
mod pvec {
    ...
    pub fn par_euclid<FM, M, FN, N>(map_m: FM, map_n: FN) -> u32
    where
        FM: FnOnce(PVecParIter<u32>) -> M,
        M: ParallelIterator<Item = u32>,
        FN: Fn(PVecParIter<u32>) -> N + Sync,
        N: ParallelIterator<Item = u32>,
    {
        let mut source = PVec::new();
        for i in 1u32..2000 {
...
fn euclid_serial(criterion: &mut Criterion) {
    ...
    group.bench_function("pvec", |b| {
        let count = pvec::euclid();
        b.iter(|| assert_eq!(pvec::euclid(), count))
    });
    group.finish();
}
...
fn euclid_faux_serial(criterion: &mut Criterion) {
    ...
    group.bench_function("std", |b| {
        ...
        b.iter(|| assert_eq!(stdvec::par_euclid(&serial, &serial), count))
    });
    group.bench_function("rrbvec", |b| {
        let count = stdvec::euclid();
        let serial = |r: RrbVecParIter<u32>| r.with_min_len(usize::MAX);
        b.iter(|| assert_eq!(rrbvec::par_euclid(&serial, &serial), count))
    });
    group.bench_function("pvec", |b| {
        let count = stdvec::euclid();
        let serial = |r: PVecParIter<u32>| r.with_min_len(usize::MAX);
        b.iter(|| assert_eq!(pvec::par_euclid(&serial, &serial), count))
    });
    group.finish();
}

fn euclid_parallel_weightless(criterion: &mut Criterion) {
    let mut group = criterion.benchmark_group("euclid_parallel_weightless");
    group.bench_function("std", |b| {
        let count = stdvec::euclid();
        b.iter(|| assert_eq!(stdvec::par_euclid_weightless(), count))
    });
    group.bench_function("rrbvec", |b| {
        let count = stdvec::euclid();
        b.iter(|| assert_eq!(rrbvec::par_euclid_weightless(), count))
    });
    group.bench_function("pvec", |b| {
        let count = stdvec::euclid();
        b.iter(|| assert_eq!(pvec::par_euclid_weightless(), count))
    });
    group.finish();
}

fn euclid_parallel_one(criterion: &mut Criterion) {
    let mut group = criterion.benchmark_group("euclid_parallel_one");
    group.bench_function("std", |b| {
        let count = stdvec::euclid();
        b.iter(|| assert_eq!(stdvec::par_euclid(|m| m, |n| n), count))
    });
    group.bench_function("rrbvec", |b| {
        let count = stdvec::euclid();
        b.iter(|| assert_eq!(rrbvec::par_euclid(|m| m, |n| n), count))
    });
    group.bench_function("pvec", |b| {
        let count = stdvec::euclid();
        b.iter(|| assert_eq!(pvec::par_euclid(|m| m, |n| n), count))
    });
    group.finish();
}
...
fn euclid_parallel_outer(criterion: &mut Criterion) {
    ...
    group.bench_function("std", |b| {
        ...
        b.iter(|| assert_eq!(stdvec::par_euclid(&parallel, |n| n), count))
    });
    group.bench_function("rrbvec", |b| {
        let count = stdvec::euclid();
        let parallel = |r: RrbVecParIter<u32>| r.with_max_len(1);
        b.iter(|| assert_eq!(rrbvec::par_euclid(&parallel, |n| n), count))
    });
    group.bench_function("pvec", |b| {
        let count = stdvec::euclid();
        let parallel = |r: PVecParIter<u32>| r.with_max_len(1);
        b.iter(|| assert_eq!(pvec::par_euclid(&parallel, |n| n), count))
    });
    group.finish();
}
...
fn euclid_parallel_full(criterion: &mut Criterion) {
    ...
    group.bench_function("std", |b| {
        let count = stdvec::euclid();
        let parallel = |r: Iter<u32>| r.with_max_len(1);
        b.iter(|| assert_eq!(stdvec::par_euclid(&parallel, &parallel), count));
    });
    group.bench_function("rrbvec", |b| {
        let count = stdvec::euclid();
        let parallel = |r: RrbVecParIter<u32>| r.with_max_len(1);
        b.iter(|| assert_eq!(rrbvec::par_euclid(&parallel, &parallel), count));
    });
    group.bench_function("pvec", |b| {
        let count = stdvec::euclid();
        let parallel = |r: PVecParIter<u32>| r.with_max_len(1);
        b.iter(|| assert_eq!(pvec::par_euclid(&parallel, &parallel), count));
    });
    group.finish();
}
